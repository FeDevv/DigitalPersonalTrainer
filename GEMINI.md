...
*   **Data Access Layer Standards**:
    ...
    *   **Fail-Fast Validation**: I DTO (Java Records) utilizzano costruttori compatti per validare i dati all'istante della creazione. La logica di validazione comune (es. email) è centralizzata in `shared.utils.ValidationUtils`.
    *   **UX - Selezione Guidata**: Le interfacce UI non richiedono mai ID "al buio". Il LogicController coordina la visualizzazione preventiva delle liste di selezione (Macchinari, Esercizi, Clienti) prima di richiedere l'input di un ID.
    *   **Verticalizzazione & Purezza**: I DAO devono essere "puri" e verticali (un DAO per tabella/entità). È severamente vietato che un DAO istanzi o utilizzi altri DAO al suo interno per evitare il pattern "Middle Man" e dipendenze circolari.    *   **Cross-DAO Coordination**: Se una funzionalità richiede l'interazione con più entità (es. il Proprietario che crea un PT), il coordinamento deve avvenire esclusivamente all'interno del **LogicController** di riferimento, che inietterà e orchestrerà i DAO verticali necessari.
