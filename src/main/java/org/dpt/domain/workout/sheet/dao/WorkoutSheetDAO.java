package org.dpt.domain.workout.sheet.dao;
...
/**
 * Data Access Object per la gestione del ciclo di vita delle Schede di Allenamento.
 * -
 * Il DAO orchestra la persistenza delle testate ('SCHEDA') e del dettaglio esercizi ('COMPOSTA').
 * Implementa logiche avanzate integrandosi con la logica procedurale del database:
 * <ul>
 *   <li><b>Stored Procedure:</b> Utilizza 'sp_crea_nuova_scheda' per garantire l'atomicità
 *       della creazione (disattivazione automatica della scheda precedente).</li>
 *   <li><b>Viste:</b> Interroga 'vw_scheda_attiva_cliente' per recuperare Read-Model arricchiti.</li>
 * </ul>
 */
public class WorkoutSheetDAO {
    private final Connection connection;

    /** Query e invocazioni di procedure per la gestione delle routine. */
    ...
    /**
     * Recupera il dettaglio completo di una specifica scheda (storica o attiva).
     * @param sheetId ID della scheda.
     * @return Lista di elementi dettagliati.
     */
    public List<ActiveSheetItem> getSheetDetails(int sheetId) {
        return getRoutineByQuery(SHEET_DETAILS, sheetId);
    }

    /** Helper per l'esecuzione di query che restituiscono Read-Model denormalizzati. */
    ...
    /** Recupera lo storico completo di tutte le schede assegnate a un cliente. */
    ...
    /** Individua l'unica scheda marcata come attiva per il cliente specificato. */
    ...
    /**
     * Crea l'intestazione di una nuova scheda delegando l'atomicità al database.
     * La stored procedure garantisce che ogni nuova scheda disattivi automaticamente la precedente.
     */
    ...
    /**
     * Associa un esercizio alla scheda definendo i parametri tecnici di esecuzione.
     */
    ...
    /** Recupera l'elenco delle schede redatte da un determinato Personal Trainer. */
